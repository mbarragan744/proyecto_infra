<script lang="ts">
    ...
    authStore.subscribe(u => currentUser = u);

    // Items y total del carrito
    $: items = cartItems.items;
    $: total = cartItems.getTotal();
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    {#if items.length === 0}
        <div class="text-center py-12 bg-secondary/50 rounded-lg">
            ...
            <Button on:click={() => goto('/')} class="bg-accent text-accent-foreground">
                Continuar comprando
            </Button>
        </div>
    {:else}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-card p-6 rounded-lg border border-border h-fit">
                ...
                <Button on:click={handleCheckout} class="w-full bg-accent text-accent-foreground hover:opacity-90">
                    Ir al Checkout
                </Button>
                <Button
                    on:click={() => goto('/')}
